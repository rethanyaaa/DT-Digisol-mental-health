...
const verifyRazorpay = async (req, res) => {
  try {
    ...
    if (orderInfo.status === "paid") {
      ...
      // Generate consultation link for video consultations
      if (appointment.consultationType === "video") {
        try {
          const { generateConsultationLink } = await import(
            "../controllers/consultationController.js"
          );
          await generateConsultationLink(
            { body: { appointmentId: appointment._id } },
            {
              status: (code) => ({
                json: (data) => {
                  if (code === 201) {
                    console.log(
                      "Consultation link generated successfully for appointment:",
                      appointment._id
                    );
                  } else {
                    console.error(
                      "Failed to generate consultation link:",
                      data
                    );
                  }
                },
          ...
          console.error(
            "Error generating consultation link:",
            consultationError
          );
          // Don't fail the payment verification if consultation link generation fails
        }
      }
