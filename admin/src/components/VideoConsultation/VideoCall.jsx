...
const VideoCall = ({ socket, roomId, userType, onEndCall }) => {
  ...
  useEffect(() => {
    ...
    console.log("VideoCall props:", { socket: !!socket, roomId, userType });
    initializeConnection();
    return () => {
  ...
  const handleEndCall = () => {
    ...
    // Notify patients that doctor ended the video call
    if (socket && userType === "doctor") {
      console.log("Emitting doctor-end-video-call event");
      socket.emit("doctor-end-video-call", {
        roomId,
